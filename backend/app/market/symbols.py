...
def seed_symbol_mappings_from_file(db: Session) -> int:
    ...
    for item in load_symbol_mapping_file():
        ...
        payload = {
            ...
            "target_notional": float(item.get("target_notional", 1000.0)),
            "max_open_notional": float(item.get("max_open_notional", 5000.0)),
            "max_open_groups": int(item.get("max_open_groups", 1)),
            "open_cooldown_seconds": int(item.get("open_cooldown_seconds", 30)),
            ...
            "maker_order_ttl_seconds": int(item.get("maker_order_ttl_seconds", item.get("hl_order_ttl_seconds", 3))),
            "maker_unfilled_action": item.get("maker_unfilled_action", "cancel"),
            "leg_a_close_order_type": item.get("leg_a_close_order_type", item.get("hl_close_order_type", "market")),
            "leg_b_close_order_type": item.get("leg_b_close_order_type", item.get("mt5_close_order_type", "market")),
