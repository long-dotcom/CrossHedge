...
- PnL 权威口径分三层：预测 PnL 使用当前可成交入场/退出价差及预计开平手续费；持仓 PnL 使用真实入场 Fill、当前可成交平仓价差、已发生手续费及预计剩余平仓手续费；已实现 PnL 使用真实开平仓 Fill 和全部实际手续费。页面不得把手续费字段单独标成“完整成本”。
- `spread_cost` 与手续费必须独立保存：统计信号继续使用可成交 entry/close 价差，禁止重复扣点差；研究展示可将两者相加，并同时提供拆分字段。
