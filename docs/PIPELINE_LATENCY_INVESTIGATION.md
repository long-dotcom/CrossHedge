...
调查阶段没有修改行情采集、扫描调度或诊断指标代码。随后按成本口径要求移除了扫描阶段的 Funding/Swap 预测及其 MT5 数据读取，并停止持仓 Funding/Swap 同步；盈亏只扣除真实开平仓手续费。由于本机 Docker/PostgreSQL 未运行，无法取得线上同一时刻的指标和日志，以下结论来自代码链路、运行配置和计时点审计。
...
部署埋点后确认，高延迟约每 10～11 秒出现一次，与原 `SIGNAL_STATS_CACHE_TTL_MS=10000` 完全一致。慢轮次中 MT5 session、报价同步、手续费和本地计算通常只有 0～4ms，主要耗时来自 `signal_history_load`：三个品种、两个方向串行读取历史价差，单组查询约 132～421ms，整轮达到 1.7～2.1 秒。缓存命中时整轮扫描仅需约 12～31ms。

根因是统计缓存有效期和后台刷新间隔共用同一个 10 秒配置。缓存到期与刷新同时发生，扫描线程会在刷新结果写回前同步回源 PostgreSQL，形成周期性缓存击穿。

现已拆分为：

- `SIGNAL_STATS_REFRESH_INTERVAL_MS=10000`：后台每 10 秒刷新；
- `SIGNAL_STATS_CACHE_TTL_MS=60000`：旧统计结果保留 60 秒；
- 为兼容已有环境变量，有效 TTL 至少为刷新间隔的三倍。

因此后台刷新期间扫描器继续使用上一版分位数、Z-score 和退出线，不再因两者同时到期而进入历史 SQL 热路径。

## 扫描轮次快照优化（2026-07-23）

后续线上采样仍观察到个别约 50～200ms 的 Redis 读取尖峰。即使单次操作最终恢复，原先按品种、按方向串行读取也会把一次共享依赖抖动直接放大到整轮扫描。扫描热路径现改为轮次快照：

- 一轮开始时用一次 `MGET` 读取所有启用品种的两腿 BBO，整轮只使用这份不可变报价；
- 一轮开始时用一次 `MGET` 读取全部“品种×方向”统计结果；同方向的退出线计算和最终利润复核复用同一份 `SignalStats`；
- MT5 交易能力和隔离状态通过一个 Redis pipeline 批量读取，整轮按内存字典查询；由于隔离状态可由独立执行进程写入，不能长期只放在扫描进程内；
- 策略配置、品种映射和 MT5 会话兜底结果改为进程内缓存。设置 API 更新后主动清空对应缓存，60 秒/30 秒 TTL 仅作为非 API 修改的兜底。

新增慢操作名为 `quote_snapshot_load`、`signal_snapshot_load`、`tradability_snapshot_load` 和 `session_snapshot_load`。因此后续若仍出现尖峰，可以直接判断慢在某一份批量快照，还是慢在纯计算/持久化阶段，不再需要从大量单品种 GET 日志反推。

## 成本与 PnL 展示口径收口（2026-07-23）

扫描热路径不额外访问外部服务。扫描结果在原有 `spread_cost` 和手续费字段之外，保存预计开仓/平仓手续费拆分；执行前复核复用扫描公式。仪表盘、对冲组和链路监控使用已保存的预计平仓费计算“当前立即平仓净 PnL”，不会为页面请求重复读取 venue 成本。价差研究的完整成本线由点差摩擦与单位手续费相加，统计信号内部仍保留原有不重复扣点差的计算口径。
